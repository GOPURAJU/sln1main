...
from .forms import ApplicationVerificationForm  # Ensure you have this form defined

def personal_verification_add_or_update(request, instance_id):
    # Get the PersonalDetail instance
    personal_detail = get_object_or_404(PersonalDetail, id=instance_id)
    
    # Try to get existing ApplicationVerification instance
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            applicant_document.save()  # Ensure the instance is saved
            
            # Check the verification status
            if applicant_document.verification_status == 'Approved':
                # Redirect to the disbursement details form
                return redirect('pldisbursement_details', verification_id=applicant_document.id)  # Use the correct keyword
            else:
                # Redirect to a different page or success page
                return redirect('success_page')  # Change this to your actual success URL
        else:
...
def dashboard(request):
    ...
    gold_loans_count = Goldloanapplication.objects.count()
    lap_loans_count = LoanApplication.objects.count()
    credit_count=CreditDetail.objects.count()
    all_ins_count=AllInsurance.objects.count()
    life_ins_count=LifeInsurance.objects.count()
    gen_ins_count=GeneralInsurance.objects.count()
    hel_ins_count=healthInsurance.objects.count()
    ...
    context = {
        ...
        'gold_loans_count': gold_loans_count,
        'lap_loans_count': lap_loans_count,
        'credit_count': credit_count,
        'all_ins_count':all_ins_count,
        'gen_ins_count':gen_ins_count,
        'hel_ins_count':hel_ins_count,
        'life_ins_count':life_ins_count
